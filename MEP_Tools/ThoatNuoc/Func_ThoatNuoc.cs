...
namespace MEP_Tools.ThoatNuoc
{
    public class Func_ThoatNuoc
    {
        ...
        public void Case1(XYZ Intersec, XYZ stPoint, XYZ edPoint, ElementId _pipeTypeID, ElementId _levelID, Element EleInput, double distance, XYZ point, Connector Conn_With_Tee, Pipe p1, Pipe p2, Pipe p3, Connector Conn_With_Elbow, XYZ newp)
        {
            ...
            double kc1 = UnitUtils.ConvertFromInternalUnits(stPoint.DistanceTo(Intersec) - distance, DisplayUnitType.DUT_MILLIMETERS); ;
            double kc2 = UnitUtils.ConvertFromInternalUnits(stPoint.DistanceTo(Intersec) + distance, DisplayUnitType.DUT_MILLIMETERS); ;
            double len = UnitUtils.ConvertFromInternalUnits(P.get_Parameter(BuiltInParameter.CURVE_ELEM_LENGTH).AsDouble(), DisplayUnitType.DUT_MILLIMETERS);
        ...
        public void Case5(Element Input_Ele, Pipe P_new, XYZ stPoint, XYZ edPoint, XYZ Intersec, double distance, ElementId _pipeSystemType, ElementId _pipeTypeID, ElementId _levelID, Connector Conn3, Connector Conn_O)
        {
            #region 'Lay thong so'
            Pipe P = Input_Ele as Pipe;
            double kc1 = UnitUtils.ConvertFromInternalUnits(stPoint.DistanceTo(Intersec) - distance/4, DisplayUnitType.DUT_MILLIMETERS); ;
            double kc2 = UnitUtils.ConvertFromInternalUnits(stPoint.DistanceTo(Intersec) + distance/4, DisplayUnitType.DUT_MILLIMETERS); ;
        ...
        public void Case6(Element Input_Ele, Element Input_Ele2, XYZ stPoint, XYZ edPoint, XYZ Intersec, double distance, ElementId _pipeSystemType, ElementId _pipeTypeID, ElementId _levelID, Pipe P_new, Connector Conn3, Connector Conn_O)
        {
            ...
            double kc1 = UnitUtils.ConvertFromInternalUnits(stPoint.DistanceTo(Intersec) - distance, DisplayUnitType.DUT_MILLIMETERS); 
            double kc2 = UnitUtils.ConvertFromInternalUnits(stPoint.DistanceTo(Intersec) + distance, DisplayUnitType.DUT_MILLIMETERS); 
            double len = UnitUtils.ConvertFromInternalUnits(P_chinh.get_Parameter(BuiltInParameter.CURVE_ELEM_LENGTH).AsDouble(), DisplayUnitType.DUT_MILLIMETERS);
